<template>
	...
	>
		...
		<div class="vw-header-days">
			<!-- TODO: ADD USERS search -->
			<div class="wv-search-bar" >
				user filter
			</div>
			<template v-for="(label, index) in dailyHours">
				<slot :index="getColumnDOWClass(index)" :label="label" name="dayHeader">
					<div
						:key="`resource-${getColumnDOWClass(index)}`"
		...
		<div :aria-multiselectable="enableDateSelection" class="vw-resources">
            <div v-for="(resource, resIndex) in getResource" 
            :key="`${resIndex}-${displayResource}-resource`"
                class="vw-resource">

				<div class="vw-res-name">
					<slot name="resourceName"
                    :resource="resource"
					><span>{{ resource.title }}</span></slot>   
				</div>

				<div class="dv-dailyhours">
                    <div
						v-for="(hr, hrIndex) in dailyHours"
						:key="`dailyhour-${getColumnDOWClass(hrIndex)}-${resIndex}`"
						:draggable="enableDateSelection"
						:class="[
							'cv-day',
							'dv-hour',
							getColumnDOWClass(hrIndex),
							'instance' + hr,
						]">
						<slot :hr="hr" name="hrContent" />
					</div>

					<template v-for="i in getDailyResources(resource, resIndex)">
                        <slot :value="i" name="item">
                            <div
								:key="`${i.id}-${i.title}-${resIndex}`"
...
export default {
	...
	props: {
        ...
        displayResource: { type: String, default: "user" },//user or job
	},
	...
	data: () => ({
        ...
        // NEVER EVER CHANGE THE FOLLOWING PROPERTIES!!!
        displayPeriodUom: "day",
	...
	computed: {
		...
		*/
		...
		periodLabel() {
			return this.formattedSingleDay(
				this.periodStart,
				this.monthNames
			)
		},
		...
		headerProps() {
			return {
				...
				previousYear: this.getIncrementedPeriod(-30),
				...
				nextYear: this.getIncrementedPeriod(30),
        ...
        getResource(){
           const resType = this.displayResource; // user or job
           if(resType == "job"){
	...
	methods: {
		...
		*/
		getIncrementedPeriod(count) {
			const newStartDate = this.incrementDay(
				this.periodStart,
				count
			)
			const newEndDate = this.incrementDay(
				newStartDate,
				this.displayPeriodCount
			)
		...
		findAndSortItemsInWeek(weekStart, resId) {
		...
		findAndSortItemsInDay(date, resId){
			// Return a list of items that INCLUDE any portion of a given week.
			return this.findAndSortItemsInCurrentDate(
				date,
                resId
			)
		},
		findAndSortItemsInCurrentDate(startDate, resId) {
            ...
            let tempFixedItems = this.fixedItems;
			const dayStart = (new Date(startDate)).setHours(0, 0, 0, 0);
			const dayEnd = (new Date(startDate)).setHours(23, 59, 59, 999);
            if(this.displayResource == "job"){
                // switch events as users
                let k = 0;
                tempFixedItems = tempFixedItems.reduce((accumulator, job) => {
                    const origItm = job.originalItem;
                    const newEvents = [];
                    origItm.users.forEach((usr) => {
                        newEvents.push({
                            classes: job.classes,
                            originalItem: job.originalItem,
                    ...
                    return [...accumulator, ...newEvents];
                },[]);
            }
            
			return tempFixedItems
                .filter((item) => {
                    if(this.displayResource == "job"){
                        return item.jobId == (+resId);
                    }
                    const origItm = item.originalItem;
                    const usrIds = origItm.users.map(usr=>usr.id);
                    
                    return usrIds,usrIds.includes(+resId);
                })
				.filter(
					(item) =>
		...
		getDailyResources(resource, resId){
			const items = this.findAndSortItemsInDay(this.showDate, resource.id);
			const dayStart = (new Date(this.showDate)).setHours(0, 0, 0, 0);
			const dayEnd = (new Date(this.showDate)).setHours(23, 59, 59, 999);
            const results = []
			const itemRows = [[], [], [], [], [], [], []]
			for (let i = 0; i < items.length; i++) {
				...
				const continued = ep.startDate < dayStart;
				const toBeContd = ep.endDate > dayEnd;


				let startOffset = continued
					? 0
					: this.dayDiff(dayStart, ep.startDate)
				let span = Math.min(
					7 - startOffset,
					this.dayDiff(this.addDays(dayStart, startOffset), ep.endDate) + 1
				)
				...
				const startHour = (new Date(ep.startDate)).getHours();
				const endHour = (new Date(ep.endDate)).getHours();
				const startMin = (new Date(ep.startDate)).getMinutes();
				const endMin = (new Date(ep.endDate)).getMinutes();
				...
				if (continued) {
					...
					if(startMin < 10){ minOffset = 0;}
					else if(startMin < 25) {minOffset = 15;}
					else if(startMin < 40) {minOffset = 30;}
					else if(startMin < 55) {minOffset = 45;}
					else if(startMin < 55) {minOffset = 0; startOffset+= 1;}
				}
				if (toBeContd) {
					...
					span= endHour;
					if(endMin < 10){ minSpan = 0;}
					else if(endMin < 25) {minSpan = 15;}
					else if(endMin < 40) {minSpan = 30;}
					else if(endMin < 55) {minSpan = 45;}
					else if(endMin < 55) {minSpan = 0; span+= 1;};

				}
				span = span-startOffset;
				minSpan=minSpan-minOffset;
				if(minSpan < 0){
					minSpan=Math.abs(minSpan);
					span--;
				}

				ep.classes.push(`offset-${startOffset}-${minOffset}`)
				ep.classes.push(`span-${span}-${minSpan}`)
				results.push(ep)
			}
			return results
		},
...
/* Metrics for items offset from start of day */
// @for $i from 0 through 96 {
// 	$iletter: $i*10/2;
// 	.cv-item.cv-item-hr.offset#{$iletter} {
// 		left: calc((#{$i * 100%} / 47)) !important;
// 	}
// }
...
@for $hour from 0 through 23 {
    @for $quarter from 0 through 3 {
        $minute: $quarter * 15;
        $totalIndex: ($hour * 4) + $quarter;
        $offsetPercentage: ($totalIndex / 96) * 100;

        .cv-item.cv-item-hr.offset-#{$hour}-#{$minute} {
            left: #{$offsetPercentage * 1%} !important;
        }
    }
}
...
@for $hour from 0 through 23 {
    @for $quarter from 0 through 3 {
        $minute: $quarter * 15;
        $totalIndex: ($hour * 4) + $quarter;
        $widthPercentage: ($totalIndex / 96) * 100;

        .cv-item.cv-item-hr.span-#{$hour}-#{$minute} {
            width: #{$widthPercentage * 1%} !important;
			//width: calc((100% / 7) - 0.05em);
        }
    }
}
.cv-item.cv-item-hr.span-24-0 {
    width: 100% !important;
}
// @for $i from 1 through 48 {
// 	$iletter: $i*10/2;
// 	.cv-item.cv-item-hr.span#{$iletter} {
// 		width: calc((#{$i * 100%} / 48)) !important;
// 		//width: calc((100% / 7) - 0.05em);
// 	}
// }
