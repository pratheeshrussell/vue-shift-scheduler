			...
			<template v-for="(label, index) in weekdayNames">
				<slot :index="getColumnDOWClass(index)" :label="label" name="dayHeader">
					<div
						...
						class="vw-header--week-day"
					>
						<div>{{ label }}</div> 
						<div>{{ currentWeekDays[index].getDate() }}</div>
					</div>
				</slot>
			</template>
		</div>
		...
		<div :aria-multiselectable="enableDateSelection" class="vw-resources">
            ...
            :key="`${resIndex}-${displayResource}-resource`"
                ...
                <div class="cv-weekdays">
                    ...
                    <!-- Add job items -->
                    <template v-for="i in getWeekResource(resource, resIndex)">
...
export default {
	...
	computed: {
		...
		*/
        ...
        // THis works since we have only 1 week to show
        currentWeekDays()
        {
            return this.daysOfWeek(this.weeksOfPeriod[0])
        },
		periodEnd() {
			return this.addDays(
	...
	methods: {
		...
		*/
		...
		findAndSortItemsInDateRange(startDate, endDate, resId) {
        ...
        getWeekResource(resource, resId){
            const items = this.findAndSortItemsInWeek(this.weeksOfPeriod[0], resource.id);
            const weekStart = this.currentWeekDays[0];
            const results = []
			const itemRows = [[], [], [], [], [], [], []]
			for (let i = 0; i < items.length; i++) {
				const ep = Object.assign({}, items[i], {
