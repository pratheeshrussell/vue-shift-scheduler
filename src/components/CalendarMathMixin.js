...
export default {
	methods: {
		...
		incrementDay(d, count) {
			let dateCopy = new Date(d);
			return dateCopy.setDate(dateCopy.getDate() + count);
		},
		...
		formattedSingleDay(startDate, monthNames) {
		...
		dayDiff(d1, d2) {
			...
			const endDate = Date.UTC(dt2.getFullYear(), dt2.getMonth(), dt2.getDate()),
				startDate = Date.UTC(dt1.getFullYear(), dt1.getMonth(), dt1.getDate())
		...
		dayDiffFull(d1, d2) {
			const dt1 = new Date(d1);
			const dt2 = new Date(d2);
			const endDate = Date.UTC(dt2),
				startDate = Date.UTC(dt1)
			return (endDate - startDate) / 86400000
		},
